...
HWND hListBox;
HWND hBackButton;
std::vector<ThreadInfo> gThreadResults;
    ...
    case WM_COMMAND:
        if ((HWND)lParam == hListBox && HIWORD(wParam) == LBN_DBLCLK)
        {
            ...
            if (sel != LB_ERR && !showingThreadMessages)
            {
                ...
                const ThreadInfo& t = gThreadResults[sel];
...
void PopulateListBoxWithThreads()
{
    ...
    for (const auto& t : gThreadResults)
        SendMessage(hListBox, LB_ADDSTRING, 0, (LPARAM)t.newest->subject.c_str());
...
// Main GUI
void ShowThreadListGUI(const std::vector<ThreadInfo>& threadResults)
{
    gThreadResults = threadResults;
